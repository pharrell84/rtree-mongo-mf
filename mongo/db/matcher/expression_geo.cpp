...
namespace mongo {
    ...
    Status GeoExpression::parseQuery(const BSONObj &obj) {
		...
		else if (BSONObj::opRTREE == matchType) {
			predicate = GeoExpression::RTREEWITHIN;
		} else if (BSONObj::opRTREEINTERSECT == matchType) {
			predicate = GeoExpression::RTREEINTERSECT;
		} else {
        ...
        while (geoIt.more()) {
			...
			log() << "parseQuery loop run" << elt.toString();
    ...
    Status GeoExpression::parseFrom(const BSONObj &obj) {
        ...
        // this for now.
		if ((GeoExpression::WITHIN == predicate || GeoExpression::RTREEWITHIN == predicate) && !geoContainer->supportsContains()) {
            return Status(ErrorCodes::BadValue,
        ...
        if (GeoExpression::INTERSECT == predicate || GeoExpression::RTREEINTERSECT == predicate) {
            if (!geoContainer->supportsProject(SPHERE)) {
                return Status(ErrorCodes::BadValue,
                              str::stream()
