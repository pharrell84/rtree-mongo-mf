...
#include "mongo/db/index/testindex_access_method.h"
...
namespace mongo {
	...
	MONGO_EXPORT_SERVER_PARAMETER(failIndexKeyTooLongTest, bool, true);

	TestIndexAccessMethod::TestIndexAccessMethod(IndexCatalogEntry* btreeState,
		SortedDataInterface* btree)
		: _btreeState(btreeState), // initializers!
	...
	bool TestIndexAccessMethod::ignoreKeyTooLong(OperationContext *txn) {
		// Ignore this error if we're on a secondary or if the user requested it
		return !txn->isPrimaryFor(_btreeState->ns()) || !failIndexKeyTooLongTest;
	}

	// Find the keys for obj, put them in the tree pointing to loc
	Status TestIndexAccessMethod::insert(OperationContext* txn,
		const BSONObj& obj,
		const RecordId& loc,
		...
		*numInserted = 0;
		...
		log() << "IN INSERT FOR TEST INDEX";
		
		BSONObjSet keys;
		// Delegate to the subclass.
		getKeys(obj, &keys); // need to implement this, or inherit this class and implement

		Status ret = Status::OK();
		for (BSONObjSet::const_iterator i = keys.begin(); i != keys.end(); ++i) {
			Status status = _newInterface->insert(txn, *i, loc, options.dupsAllowed); // insert into the sorteddatainterface
			// the only interface other files seem to support are btree, so this has to be BtreeInterfaceImpl unless you change it in btree_interface.cpp
	...
	void TestIndexAccessMethod::removeOneKey(OperationContext* txn,
		const BSONObj& key,
		const RecordId& loc,
		bool dupsAllowed) {
		try {
	...
	Status TestIndexAccessMethod::newCursor(OperationContext* txn, const CursorOptions& opts, IndexCursor** out) const {
		*out = new BtreeIndexCursor(_newInterface->newCursor(txn, opts.direction));
		return Status::OK();
	}

	// Remove the provided doc from the index.
	Status TestIndexAccessMethod::remove(OperationContext* txn,
		const BSONObj &obj,
		const RecordId& loc,
		const InsertDeleteOptions &options,
		int64_t* numDeleted) {
	...
	Status TestIndexAccessMethod::initializeAsEmpty(OperationContext* txn) {
		return _newInterface->initAsEmpty(txn);
	}

	Status TestIndexAccessMethod::touch(OperationContext* txn, const BSONObj& obj) {
		BSONObjSet keys;
		getKeys(obj, &keys);
	...
	Status TestIndexAccessMethod::touch(OperationContext* txn) const {
		return _newInterface->touch(txn);
	}

	RecordId TestIndexAccessMethod::findSingle(OperationContext* txn, const BSONObj& key) const {
		boost::scoped_ptr<SortedDataInterface::Cursor> cursor(_newInterface->newCursor(txn, 1));
	...
	Status TestIndexAccessMethod::validate(OperationContext* txn, bool full, int64_t* numKeys,
	...
	bool TestIndexAccessMethod::appendCustomStats(OperationContext* txn,
		BSONObjBuilder* output,
		double scale) const {
		return _newInterface->appendCustomStats(txn, output, scale);
	}

	long long TestIndexAccessMethod::getSpaceUsedBytes(OperationContext* txn) const {
		return _newInterface->getSpaceUsedBytes(txn);
	}

	Status TestIndexAccessMethod::validateUpdate(OperationContext* txn,
		const BSONObj &from,
		const BSONObj &to,
		...
		TestIndexPrivateUpdateData *data = new TestIndexPrivateUpdateData();
	...
	Status TestIndexAccessMethod::update(OperationContext* txn,
		...
		TestIndexPrivateUpdateData* data =
			static_cast<TestIndexPrivateUpdateData*>(ticket._indexSpecificUpdateData.get());
	...
	IndexAccessMethod* TestIndexAccessMethod::initiateBulk(OperationContext* txn) {
		return NULL;
	}

	Status TestIndexAccessMethod::commitBulk(IndexAccessMethod* bulkRaw,
		bool mayInterrupt,
		bool dupsAllowed,
	...
	void TestIndexAccessMethod::getKeys(const BSONObj& obj, BSONObjSet* keys) const {
